...
def process_img_poly2(img_path, label_json):
    poly_dict = {}
    x_data = [] 
    img_obj = Image.open(img_path)
    ...
    for feat in label_data['features']['xy']:
        ...
        polygon_geom = shapely.wkt.loads(feat['wkt'])
        polygon_pts = np.array(list(polygon_geom.exterior.coords))
        poly_img = process_img(img_array, polygon_pts, 0.8) #returns img_array[ymin:ymax, xmin:xmax, :]

        #I think i'll store the poly_img with the poly_uuid in a dict
        poly_dict[poly_uuid]=poly_img
